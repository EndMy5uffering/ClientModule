		...
		if(templateTypes.contains(t)) {
			...
			while(resultData.next()) {
				...
				try {
					...
					Client.getLogger().log(Level.ERROR, "Could not cast " + tocast.getClass().getSimpleName() + " to " + t.getSimpleName());
		...
		if(templateTypes.contains(columnType)) {
			...
			while(resultData.next()) {
				...
				try {
					...
					Client.getLogger().log(Level.ERROR, "Could not cast " + tocast.getClass().getSimpleName() + " to " + columnType.getSimpleName());
					return new ArrayList<>();
				}
			}
		}else {
		...
		for(Field f : newInstance.getClass().getDeclaredFields()) {
			...
			if(f.isAnnotationPresent(DatabaseValue.class)) {
				if(templateTypes.contains(f.getType())) {
					...
					Client.getLogger().log(Level.ERROR, "Can not read field of type: " + f.getType().getSimpleName() + " as value type in: " + newInstance.getClass().getSimpleName());
				}
			}
		}
		return newInstance;
