...
public class QueryObject {
		...
		for(Field f : o.getClass().getDeclaredFields()) {
			...
			if(f.isAnnotationPresent(DatabaseValue.class)) {
				if(templateTypes.contains(f.getType())) {
					...
					Client.getLogger().log(Level.ERROR, "Can not read field of type: " + f.getType().getSimpleName() + " as value type in: " + o.getClass().getSimpleName());}
			}
		}
				...
				if(inSameGroup(groupsOfField, m.getAnnotation(DatabaseValue.class).groups())) {
					if(m.getParameterCount() <= 0) {
						if(templateTypes.contains(m.getReturnType())) {
							...
							Client.getLogger().log(Level.ERROR, "QueryObject error for: " + o.getClass().getSimpleName());
							Client.getLogger().log(Level.ERROR, "Functions with @DatabaseValue can not have return type: " + m.getReturnType().getSimpleName());
						}
					}else {
						Client.getLogger().log(Level.ERROR, "Can not read function with @DatabaseValue annotation because it has to many argumetns.");
						Client.getLogger().log(Level.ERROR, "Functions with the @DatabaseValue annotation can not have any arguments");
					}
				}
			}
		}
		...
		for(Field f : o.getClass().getDeclaredFields()) {
			...
			if(f.isAnnotationPresent(DatabaseArgument.class)) {
				if(templateTypes.contains(f.getType())) {
					...
					Client.getLogger().log(Level.ERROR, "Can not read field of type: " + f.getType().getSimpleName() + " as argument type in: " + o.getClass().getSimpleName());
				}
			}
		}
				...
				if(inSameGroup(groupsOfField, m.getAnnotation(DatabaseArgument.class).groups())) {
					if(m.getParameterCount() <= 0) {
						if(templateTypes.contains(m.getReturnType())) {
							...
							Client.getLogger().log(Level.ERROR, "QueryObject error for: " + o.getClass().getSimpleName());
							Client.getLogger().log(Level.ERROR, "Functions with @DatabaseArgument can not have return type: " + m.getReturnType().getSimpleName());
						}
					}else {
						Client.getLogger().log(Level.ERROR, "Can not read function with @DatabaseArgument annotation because it has to many argumetns.");
						Client.getLogger().log(Level.ERROR, "Functions with the @DatabaseArgument annotation can not have any arguments");
					}
				}
			}
		}
	...
	private Object getValue(Field f, Object o) {
		try {
			...
			Client.getLogger().log(Level.ERROR, "Could not get value form field: " + f.getName());
